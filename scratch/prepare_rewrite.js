...
interface MatrixItem {
  ...
  math_form: string;
  topic: string;
  question_type: string; // TN, DS, NLC
    ...
    <div className="flex h-screen bg-[#f8fafc] overflow-hidden text-gray-800">
      <div className="flex-1 flex flex-col h-full overflow-hidden p-6 gap-4">
        ...
        <div className="flex flex-1 gap-4 overflow-hidden min-h-0">
          ...
          <div className="w-[40%] bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col overflow-hidden flex-shrink-0">
                      ...
                      {isTopicExpanded && (
                        <div className="space-y-3 pl-3 border-l-2 border-indigo-50 ml-3 mt-2">
                          {Object.entries(lessons).map(([lesson, types]) => {
                            ...
                            return (
                            <div key={lesson} className="bg-gray-100/50 rounded-lg overflow-hidden">
                              <div 
                                className="font-semibold text-gray-700 text-[13px] flex items-center gap-2 cursor-pointer bg-gray-200/50 hover:bg-gray-200 p-2 transition-colors"
                                  ...
                                  {Object.entries(types).map(([typeName, forms]) => {
                                    ...
                                    return (
                                      <div key={typeName} className="mb-2 last:mb-0">
                                        <div 
                                          className="text-[13px] text-teal-800 font-semibold flex items-center gap-2 cursor-pointer hover:text-teal-600 mb-1"
                                        ...
                                        {isTypeExpanded && (
                                          <div className="space-y-1 pl-6">
                                            {forms.map(({cat, inv}, idx) => {
                                              const formKey = \`\${cat.math_form}_\${inv.question_type}_\${inv.difficulty}\`;
                                              const isChecked = matrixItems.some(m => m.id === formKey);
                                              return (
                                                <label 
                                                  key={idx} 
                                                  className={\`flex items-start gap-2 p-1.5 rounded cursor-pointer transition-colors \${isChecked ? 'bg-indigo-50' : 'hover:bg-gray-50'}\`}
                                                >
                                                  <input 
                                                    type="checkbox" 
                                                    className="mt-1 flex-shrink-0 cursor-pointer w-3.5 h-3.5 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                                                    checked={isChecked}
                                                    onChange={() => toggleMatrixItem(cat, inv)}
                                                  />
                                                  <div className="text-[13px] text-gray-700 leading-tight">
                                                    <span className="text-gray-500 mr-1">[{getDiffName(inv.difficulty)}]</span>
                                                    {cat.math_form} 
                                                    <span className="text-gray-400 ml-1">(Kho: {inv.count})</span>
                                                  </div>
                                                </label>
                                              );
                                            })}
          ...
          <div className="w-[60%] bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col overflow-hidden">
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm p-4 flex justify-center animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl w-full max-w-5xl h-full flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-4 bg-white border-b border-gray-100 flex items-center gap-3">
              ...
              <button onClick={handleExportWord} className="bg-blue-600 text-white px-4 py-2.5 rounded-lg font-bold flex items-center gap-2 text-sm shadow-sm hover:bg-blue-700">
...
`;
fs.writeFileSync('scratch/rewrite_exams.js', \`
const fs = require('fs');
fs.writeFileSync('src/app/admin/exams/page.tsx', \`\${code}\`);
console.log('Successfully rewrote exams/page.tsx');
\`);
