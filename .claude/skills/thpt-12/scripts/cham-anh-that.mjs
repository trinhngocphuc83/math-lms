  ...
  if (kMoi) {
    ...
    if (luoi) {
      ...
      if (kq.timDuocNeo) {
        ...
        tDocMoi += docMoi; tSotMoi += Math.max(0, sotMoi); tNgoMoi += ngoMoi;
        cot = `  ${String(cu.doc).padStart(3)}/${String(cu.sot).padStart(3)}/${String(cu.ngo).padStart(3)}   `
            + ` |   ${String(docMoi).padStart(3)}/${String(Math.max(0, sotMoi)).padStart(3)}/${String(ngoMoi).padStart(3)}`;
...
console.log(`     MỚI đọc ${tDocMoi} · bỏ sót ${tSotMoi} · báo ngờ ${tNgoMoi}`);
rmSync(TAM, { recursive: true, force: true });
