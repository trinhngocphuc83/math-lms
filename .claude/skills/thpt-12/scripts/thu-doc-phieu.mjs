...
const thu = (ten, to, opt, mong) => {
  ...
  const doc = Object.keys(kq.traLoi).length, ngo = kq.khongChac.length;
  const dat = mong(kq);
  console.log(`   ${dat ? '✓' : '✗'} ${ten.padEnd(42)} đọc ${String(doc).padStart(2)} · ngờ ${String(ngo).padStart(2)}`
    + (dat ? '' : `   <- KHÔNG ĐẠT`));
  return dat;
};
...
/* 3. Vết bẩn mờ rải rác: không được thành đáp án */
{
  const to = {};
  const ban = luoi.o.filter((_, i) => i % 37 === 0).slice(0, 8);
  for (const o of ban) to[o.ma] = 0.15;
  datHet &= thu('vết bẩn mờ 8 chỗ, không tô gì', to, {}, kq => Object.keys(kq.traLoi).length === 0);
}
