...
export function cleanLatexControlChars(s: string): string {
    ...
    let res = s;
    // Sửa literal \n (2 ký tự) thành xuống dòng, trừ khi nó là lệnh LaTeX
    res = res.replace(/\\n(?!(eq|otin|exists|eg|abla|u|i|earrow|atural|parallel)\b)/g, '\n');
    
    // Nếu có mã điều khiển 0x0A (Line Feed) + lệnh thì fix lại thành backslash + lệnh
    res = res.replace(/\x0A(eq|otin|exists|eg|abla|u|i|earrow|atural|parallel)\b/g, '\\n$1');
