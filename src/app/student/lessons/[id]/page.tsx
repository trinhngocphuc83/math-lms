    ...
    <div className="my-8 bg-white border-2 border-indigo-200 rounded-[2rem] p-6 md:p-8 shadow-[6px_6px_0px_0px_rgba(199,210,254,1)] relative transition-all hover:-translate-y-1 hover:shadow-[8px_8px_0px_0px_rgba(199,210,254,1)]">
      <div className="mb-6">
         <div className="flex justify-center mb-4">
             <span className="text-indigo-800 bg-indigo-100 border-2 border-indigo-200 px-5 py-1.5 rounded-full text-sm font-black tracking-wide uppercase shadow-sm">
                 Câu hỏi tương tác
             </span>
         </div>
         ...
         <div className="flex flex-col gap-3 mt-4">
            <input 
               ...
               className={`w-full p-4 rounded-xl border-2 outline-none font-bold text-gray-700 focus:ring-4 focus:ring-indigo-500/20 transition-all ${
                  isChecked 
                    ? ((normalizeAnswer(shortAnswerText || '') !== '' && normalizeAnswer(shortAnswerText || '') === normalizeAnswer(data.exactAnswer || data.correctAnswer || '')) ? 'border-green-500 bg-green-50 text-green-700' : 'border-red-500 bg-red-50 text-red-700')
