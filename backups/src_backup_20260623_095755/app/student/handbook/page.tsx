...
export default function StudentHandbook() {
  ...
  const renderFormulaPage = (formula: any, index: number) => {
    ...
    return (
      <div 
        key={formula.id} 
        className={`flipbook-page absolute inset-0 p-6 md:p-10 flex flex-col ${animClass}`}
      >
        ...
        <div className="flex justify-between items-start mb-6 pl-8">
          ...
          <span className="page-number">Trang {pageNum}/{totalPages}</span>
        </div>

        {/* Tiêu đề công thức */}
        <h3 className="text-xl md:text-2xl font-bold text-gray-800 mb-5 pl-8 flex items-center gap-3">
          <span className="inline-flex items-center justify-center w-8 h-8 bg-violet-100 text-violet-600 rounded-lg text-sm font-bold shrink-0">
            {pageNum}
          </span>
          {formula.title}
        </h3>
        ...
        <div className="flex-1 pl-8 flex flex-col">
          <div className="flex gap-5 items-start flex-1">
            ...
            {formula.image_url && (
              <div className="shrink-0 border-2 border-amber-200/60 rounded-xl overflow-hidden bg-white shadow-md max-w-[220px]">
                <img 
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">Bách khoa toàn thư Toán học</p>
            </div>
          </div>
          <Link 
            href="/student" 
      ...
      <div className="max-w-7xl mx-auto px-4 pb-8">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-200/60 shadow-sm p-4 sticky top-6 max-h-[calc(100vh-120px)] overflow-y-auto">
              <h2 className="font-bold text-violet-700 mb-4 px-2 uppercase text-[11px] tracking-widest flex items-center gap-2">
                <Library className="w-3.5 h-3.5" /> Mục Lục
              </h2>
