...
export default function NewOnlineExamPage() {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto font-sans pb-32">
      <div className="flex items-center gap-4 mb-8">
        <Link href="/admin/online-exams" className="p-2 hover:bg-slate-100 rounded-full transition-colors">
        ...
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium">Thiết lập cấu hình và tải đề thi lên hệ thống.</p>
        </div>
      </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            ...
            <div className="flex gap-2 p-1 bg-slate-100 rounded-xl mb-4">
              <button 
                ...
                className={`flex-1 py-2 px-2 rounded-lg font-bold text-sm transition-all flex items-center justify-center gap-1 ${importMethod === "AI" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
              >
                <UploadCloud className="w-4 h-4" /> Import AI
              </button>
              <button 
                onClick={() => setImportMethod("MANUAL")}
                className={`flex-1 py-2 px-2 rounded-lg font-bold text-sm transition-all flex items-center justify-center gap-1 ${importMethod === "MANUAL" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
              >
                <Code2 className="w-4 h-4" /> Nhập JSON thủ công
              </button>
              <button 
                onClick={() => setImportMethod("BANK")}
                className={`flex-1 py-2 px-2 rounded-lg font-bold text-sm transition-all flex items-center justify-center gap-1 ${importMethod === "BANK" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
              >
                <Database className="w-4 h-4" /> Từ Ngân hàng
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <datalist id="exam-titles">
                  <option value="Kiểm tra thường xuyên" />
                  <option value="Kiểm tra GK I" />
                  <option value="Kiểm tra CK I" />
                  <option value="Kiểm tra GK II" />
                  <option value="Kiểm tra CK II" />
                  <option value="Thi Thử lên 10" />
                  <option value="Thi thử vào Chuyên" />
                  <option value="Thi thử TN THPT" />
                  <option value="Thi thử ĐGNL" />
                </datalist>
              </div>
